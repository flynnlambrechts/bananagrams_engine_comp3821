...
from constants import ANCHOR_IS_PREFIX, ANCHOR_IS_SUFFIX, HORIZONTAL, MAX_LIMIT, NO_SPACE_FOR_WORD, VERTICAL, pair_start_count, pair_end_count
...
class TwoLetterJunkStrandingPlayer(StrandingPlayer):
    ...
    def find_strand_extending_anchors(self):
        ...
        for tile in self.board.tiles.values():
            if tile.get_parent(VERTICAL) == None:
            ...
            elif tile.get_parent(HORIZONTAL) == None:
    ...
    def play_best_strand_extension(self, anchors: list[Tile]) -> Word | None:
        ...
        for anchor in anchors:
            ...
            if anchor.get_parent(VERTICAL) == None:
                parent = anchor.get_parent(HORIZONTAL)
            else:
                parent = anchor.get_parent(VERTICAL)
    ...
    def play_right_angle_word(self):
        ...
        for anchor in self.board.tiles.values():
            if any(lim == MAX_LIMIT for lim in anchor.lims.lims):
                if anchor.get_parent(HORIZONTAL) == None:
                ...
                elif anchor.get_parent(VERTICAL) == None:
