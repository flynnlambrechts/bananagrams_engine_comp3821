...
'''
junk_anchor_synergy = {(j, a): _min_distribution(j, a) / letter_pair_frequency[a] for j in letter_distribution.keys() for a in letter_distribution.keys() if j in two_letter_peers[a]}
...
def best_anchor_candidates(junk: str, anchors: list[Tile]):
    ...
    candidates = [(junk_anchor_synergy[junk, a.char], a) for a in anchors if a.char in two_letter_peers[junk]]
